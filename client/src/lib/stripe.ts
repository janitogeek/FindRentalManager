...
export const getSubscription = async (subscriptionId: string) => {
  try {
    const response = await fetch(`/api/stripe?subscriptionId=${subscriptionId}`);
...
export const getCustomerSubscriptions = async (customerId: string) => {
  try {
    const response = await fetch(`/api/stripe?customerId=${customerId}&action=subscriptions`);
...
export const getInvoice = async (invoiceId: string) => {
  try {
    const response = await fetch(`/api/stripe?invoiceId=${invoiceId}`);
